                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <select id="devise" name="devise" class="form-control">
                                ...
                                <option value="GNF" {{ old('devise', $deviseUser) == 'GNF' ? 'selected' : '' }}>Franc Guinéen (GNF)</option>
                            </select>
                    ...
                    <div class="card">
                        ...
                        <div class="email-repeater mb-3">
                            <div data-repeater-list="designations">
                                @php
                                    $oldDesignations = old('designations', session('data.designations', []));
                                    // Si vide, on crée un tableau avec un élément vide pour afficher une ligne par défaut
                                    if(empty($oldDesignations)) {
                                        $oldDesignations = [['description' => '', 'quantity' => 1, 'price' => '', 'discount' => 0, 'net_price' => '', 'total' => '']];
                                    }
                                @endphp
                                
                                @foreach($oldDesignations as $index => $designation)
                                <div data-repeater-item class="row mb-3">
                                    <div class="col-md-3 mt-3 mt-md-0">
                                        <select class="select2 form-control designation" name="designations[{{ $index }}][description]">
                                            <option value="">Sélectionner</option>
                                            @foreach ($designations as $design)
                                                <option value="{{ $design->description }}" 
                                                    data-id="{{ $design->id }}" 
                                                    data-price="{{ $design->prix_unitaire }}"
                                                    {{ $designation['description'] == $design->description ? 'selected' : '' }}>
                                                    {{ $design->description }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <input type="hidden" name="designations[{{ $index }}][id]" class="designation-id" value="{{ $designation['id'] ?? '' }}">
                                    </div>
                                    <div class="col-md-1 mt-3 mt-md-0">
                                        <input type="number" class="form-control quantity" name="designations[{{ $index }}][quantity]" placeholder="Qte" 
                                            value="{{ $designation['quantity'] ?? 1 }}" min="1">
                                    </div>
                                    <div class="col-md-2 mt-3 mt-md-0">
                                        <input type="number" step="0.01" class="form-control price" name="designations[{{ $index }}][price]" 
                                    ...
                                    <div class="col-md-2 mt-3 mt-md-0">
                                        <input type="number" step="0.01" class="form-control net-price" name="designations[{{ $index }}][net_price]" 
                                            placeholder="PU Net" value="{{ $designation['net_price'] ?? '' }}" readonly>
                                    </div>
                                    <div class="col-md-2 mt-3 mt-md-0">
                                        <input type="text" step="0.01" class="form-control total" name="designations[{{ $index }}][total]" 
                                            placeholder="Total" value="{{ $designation['total'] ?? '' }}" readonly>
                                    </div>
                                    <div class="col-md-1 mt-3 mt-md-0">
