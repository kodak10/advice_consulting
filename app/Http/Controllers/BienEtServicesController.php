...
class BienEtServicesController extends Controller
{
    ...
    {
        $bien_et_services = bien_et_services::orderBy('motif', 'asc')->get();
        return response()->json($bien_et_services);
    }
    ...
    public function store(Request $request)
    {
        ...
        $bien_et_services = bien_et_services::create([
        ...
        if ($request->files->count() > 0 )
        {
            ...
            foreach ($files as $file) {
                ...
                $document = Document::create([
                    'type' => 'justificatif',
                    'nom' => 'justificatif_demande_' . $bien_et_services->id.'_'.now(),
                    'chemin_doc' => $fileData['filePath'],
                    'user_id' => 1,
                    'demande_id' => $bien_et_services->id,
                ]);
            }
    ...
    public function notificationupdate(Request $request, $notification)
    {
        ...
        foreach ($notificationsArray as $notificationId) {
            ...
            $demande_recupere = bien_et_services::find($notification->demande_id);
            if (!$demande_recupere) {
                $results[] = [
                    'id' => $notification->id,
                    'error' => 1,
            ...
            if ($request->statut == 2 ) {
                ...
                bien_et_services::where('id', $notification->demande_id)->update([
                    'motif' => $request->motif,
                ]);
            ...
            if ($request->statut == 1) {
                $demandesExistantes = bien_et_services::where('type_demandes_id', 3)
    ...
    public function autonotifcationupdate($id)
    {
        ...
        $demande_recupere = bien_et_services::where('id', $id)->first();
    ...
    public function analyseshow(Request $request, $notification)
    {
        ...
        $demandes = bien_et_services::whereIn('id', $ids_demande_notif)->get();
    ...
    public function notification($demande_enregistre_id)
    {
        ...
        $demande_recupere = bien_et_services::find($demande_enregistre_id);
     ...
     */
    public function show(bien_et_services $bien_et_services)
    {
        //
    }
     ...
     */
    public function edit(bien_et_services $bien_et_services)
    {
        //
    }
     ...
     */
    public function destroy(bien_et_services $id)
    {
        $id->delete();
