...
class AbsencesController extends Controller
{
    ...
    {
        $absence = absences::orderBy('motif', 'asc')->get();
        ...
        if ($request->files->count() > 0 )
        {
            ...
            foreach ($files as $file) {
                ...
                $document = Document::create([
                    'type' => 'justificatif',
                    'nom' => 'justificatif_demande_' . $absence->id.'_'.now(),
                    'chemin_doc' => $fileData['filePath'],
                    'user_id' => 1,
                    'demande_id' => $absence->id,
                ]);
            }
    ...
    public function notificationupdate(Request $request, $notification)
    {
        ...
        foreach ($notificationsArray as $notificationId) {
            ...
            $demande_recupere = absences::find($notification->demande_id);
            if (!$demande_recupere) {
                $results[] = [
                    'id' => $notification->id,
                    'error' => 1,
            ...
            if ($request->statut == 2 ) {
                ...
                absences::where('id', $notification->demande_id)->update([
                    'motif' => $request->motif,
                ]);
            ...
            if ($request->statut == 1) {
                $demandesExistantes = absences::where('type_demandes_id', 3)
    ...
    public function autonotifcationupdate($id)
    {
        ...
        $demande_recupere = absences::where('id', $id)->first();
    ...
    public function analyseshow(Request $request, $notification)
    {
        ...
        $demandes = absences::whereIn('id', $ids_demande_notif)->get();
    ...
    public function notification($demande_enregistre_id)
    {
        ...
        $demande_recupere = absences::find($demande_enregistre_id);
     ...
     */
    public function update(Request $request, $absences)
    {
        $absences = absences::findOrFail($absences);
        ...
        $absences->motif_permi = $validated['motif_permi'];
        $absences->motif = $validated['motif'];
        $absences->date_depart = $validated['date_depart'];
        $absences->date_fin = $validated['date_fin'];
        $absences->lieu_travail = $validated['lieu_travail'];
        $absences->user_id = $validated['user_id'];
        $absences->direction_id = $validated['direction_id'];
        $absences->filliale_id = $validated['filliale_id'];
        ...
        if ($request->files->count() > 0 )
        {
            ...
            foreach ($files as $file) {
                ...
                $document = Document::update([
                    'type' => 'justificatif',
                    'nom' => 'justificatif_demande_' . $absences->id.'_'.now(),
                    'chemin_doc' => $fileData['filePath'],
                    'user_id' => 1,
                    'demande_id' => $absences->id,
                ]);
            }
    ...
    public function updateStatut(Request $request, $id)
    {
        ...
        $absences->statut = $request->statut;
        $absences->save();
     ...
     */
    public function destroy(absences $id)
    {
        $id->delete();
