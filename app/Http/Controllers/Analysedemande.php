...
class Analysedemande extends Controller
{
   public function index()
{
    $absences = absences:: // charge la relation client
       // ->where('pays_id', Auth::user()->pays_id)
        //->where('status', 'En Attente de facture')
        orderBy('created_at')
        ->get();
    $bienetservice = bien_et_services::
    // charge la relation client
    // ->where('pays_id', Auth::user()->pays_id)
    //->where('status', 'En Attente de facture')
    orderBy('created_at')
    ->get();
    $conger = conger:: // charge la relation client
       // ->where('pays_id', Auth::user()->pays_id)
        //->where('status', 'En Attente de facture')
        orderBy('created_at')
        ->get();
    $permission = demandepermissions:: // charge la relation client
       // ->where('pays_id', Auth::user()->pays_id)
        //->where('status', 'En Attente de facture')
        orderBy('created_at')
        ->get();
    ...
    return response()->json($absences, $bienetservice, $conger, $permission);
}
...
public function show($id)
{
    ...
    private function generatePdf(Devis $devis)
    {
        try {
            ...
            $pdf = PDF::loadView('frontend.pdf.travelrequest', compact('devis', 'client', 'banque', 'details'))
                     ->setPaper('a4', 'portrait');

            $pdfOutput = $pdf->output();

            $fileName = 'Travel-' . $devis->id . '.pdf';
            $directory = 'pdf/devis';
