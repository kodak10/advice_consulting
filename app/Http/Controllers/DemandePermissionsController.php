...
class DemandePermissionsController extends Controller
{
    ...
    {
        $demandepermissions = demandepermissions::orderBy('motif', 'asc')->get();
        return response()->json($demandepermissions);
    }
    ...
    public function store(Request $request)
    {
        ...
        $demandepermissions = demandepermissions::create([
        ...
        if ($request->files->count() > 0 )
        {
            ...
            foreach ($files as $file) {
                ...
                $document = Document::create([
                    'type' => 'justificatif',
                    'nom' => 'justificatif_demande_' . $demandepermissions->id.'_'.now(),
                    'chemin_doc' => $fileData['filePath'],
                    'user_id' => 1,
                    'demande_id' => $demandepermissions->id,
                ]);
            }
    ...
    public function notificationupdate(Request $request, $notification)
    {
        ...
        foreach ($notificationsArray as $notificationId) {
            ...
            $demande_recupere = demandepermissions::find($notification->demande_id);
            if (!$demande_recupere) {
                $results[] = [
                    'id' => $notification->id,
                    'error' => 1,
            ...
            if ($request->statut == 2 ) {
                ...
                demandepermissions::where('id', $notification->demande_id)->update([
                    'motif' => $request->motif,
                ]);
            ...
            if ($request->statut == 1) {
                $demandesExistantes = demandepermissions::where('type_demandes_id', 3)
    ...
    public function autonotifcationupdate($id)
    {
        ...
        $demande_recupere = demandepermissions::where('id', $id)->first();
    ...
    public function analyseshow(Request $request, $notification)
    {
        ...
        $demandes = demandepermissions::whereIn('id', $ids_demande_notif)->get();
    ...
    public function notification($demande_enregistre_id)
    {
        ...
        $demande_recupere = demandepermissions::find($demande_enregistre_id);
     ...
     */
    public function show(demandepermissions $demandepermissions)
    {
        //
    }
     ...
     */
    public function edit(demandepermissions $demandepermissions)
    {
        //
    }
     ...
     */
    public function destroy(demandepermissions $id)
    {
        $id->delete();
